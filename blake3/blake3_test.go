...
func TestHasher_Vectors(t *testing.T) {
	check := func(t *testing.T, h B3hasher, input []byte, hash string) {
		// ensure reset works
	...
	t.Run("Keyed", func(t *testing.T) {
		for _, tv := range vectors {
			h := B3hasher{Flags: consts.Flag_Keyed}
	...
	t.Run("DeriveKey", func(t *testing.T) {
		var buf [32]byte
		for _, tv := range vectors {
			h := B3hasher{Flags: consts.Flag_DeriveKeyContext, Key: consts.IV}
